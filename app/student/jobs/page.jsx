...
import { Container, Typography, Grid, Card, CardContent, Box, Button, TextField, MenuItem, Select, FormControl, InputLabel, Chip } from "@mui/material";
...
export default function StudentJobs() {
  ...
  const fetchJobs = async () => {
    ...
    if (province) params.set("country", "ایران"); // برای فیلتر فعلی API
  ...
  return (
    <Container sx={{ py:4 }}>
      ...
      <Grid container spacing={2} sx={{ mb:2 }}>
        <Grid item xs={12} md={3}><TextField label="عنوان" fullWidth value={q} onChange={e=>setQ(e.target.value)} /></Grid>
        <Grid item xs={6} md={2}>
          <FormControl fullWidth>
            <InputLabel>استان</InputLabel>
            <Select label="استان" value={province} onChange={e=>{ setProvince(e.target.value); setCity(""); }}>
        ...
        <Grid item xs={6} md={2}><TextField type="date" label="از تاریخ" InputLabelProps={{shrink:true}} fullWidth value={from} onChange={e=>setFrom(e.target.value)} /></Grid>
        <Grid item xs={6} md={2}><TextField type="date" label="تا تاریخ" InputLabelProps={{shrink:true}} fullWidth value={to} onChange={e=>setTo(e.target.value)} /></Grid>
        <Grid item xs={12} md={1}><Button fullWidth variant="contained" onClick={fetchJobs}>فیلتر</Button></Grid>
      </Grid>
